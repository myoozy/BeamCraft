...
**Actuators and other constraint systems**: `AdaptiveDamperActuators`/`AdaptiveDamperController`/`AdaptiveDamperParser` (thread-safe adaptive-damper lifecycle and mode switching, with the source-adapted coefficient kernel isolated in `AdaptiveDamperCompatibility`), `HydroActuatorController` + `HydroContainer` (hydro actuators), `TorsionHydroContainer` (torsion hydro steering), `CouplerContainer`/`CouplerRegistry` (modern coupler system), `TorqueReactionSolver`.
...
- `JBeamParser` — Parses cleaned JSON into raw data maps, with `$=...` expression evaluation via the independent `JBeamExpressionEvaluator`/`BeamExpressionContext`; the source-adapted JBeam function environment is isolated in `JBeamExpressionBuiltins`.
...
- Specialized parsers: `JBeamPressureWheelsParser` (reads pressure-wheel tables and delegates rim/tire topology construction to `PressureWheelBuilder`), `JBeamCameraParser`, `powertrain/JBeamPowertrainParser`.
...
**`DaeMeshLoader.java`** — Uses LWJGL Assimp to load COLLADA (`.dae`) files. Caches parsed `RawGeometry` by filename with ref-counting. Supports zip bundles and a "common" shared mesh library. A mesh is resolved **by name on demand**: flexbody and prop tables name a mesh but not the file it lives in, so candidate DAE files are scanned for node and geometry names (cached per file) and only the providers are imported; the vehicle's own namespace is still imported whole, and a name no file declares is reported rather than chased. Raw geometry retains the DAE object origin for rigid-prop pivots.

**`FlexbodyBindingUtil.java`** — Binds deforming mesh vertices to physics nodes. For each mesh vertex, finds the nearest N physics nodes (by position) and computes skinning weights + the local offset basis. `PropMeshBinder` separately binds rigid prop meshes to their three synthetic render nodes. The resulting data is uploaded as TBO textures for the GPU skinning pipeline.
