...
**`PhysicsWorld.java`** — Core simulation controller. Owns the vehicle list and the globally shared `VoxelSnapshot`, `DynamicAxisSweep` (part-segmented node SAP broadphase), `SoftBodyCollisionManager` and `CollisionPipeline`. The SAP collects swept node and triangle bounds by `(vehicle, part)`, sorts each node-part segment independently, and queries only node parts whose bounds can overlap the current triangle part; invalid or unknown part metadata falls back to the conservative legacy query. Key constants: `invPhysicsDT = 2000`, `RENDER_SNAPSHOT_SUBSTEP_INTERVAL = 10` (snapshots at 200 Hz of simulated time), `ELECTRIC_SNAPSHOT_SUBSTEP_INTERVAL = 10`. A step is split in two so that **no Minecraft world access happens on the physics thread**: `prepareStep` captures the voxel snapshot and other world-derived state on the client thread, `simulatePreparedStep` runs the substep loop.
