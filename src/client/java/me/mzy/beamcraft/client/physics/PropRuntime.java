...
public final class PropRuntime {
    ...
    private PropRuntime() {}
    private static final float[] referenceBasis = new float[9];
    private static final float[] propOrientation = new float[9];
    private static final float[] globalScratch = new float[9];
    public static void appendRenderNodes(PropContainer props, SoftBodyVehicle vehicle, float[] x, float[] y, float[] z,
                                  int physicsNodeCount) {
        float steeringDegrees = steeringWheelDegrees(vehicle, x, y, z);
        for (int prop = 0; prop < props.count; prop++) {
            int syntheticBase = props.syntheticNodeBase(physicsNodeCount, prop);
            float value = functionValue(props, vehicle, prop, steeringDegrees);
            boolean orientationReady = props.hasBaseRotationGlobal[prop]
                    ...
                    props.rotationX[prop] * value, props.rotationY[prop] * value, props.rotationZ[prop] * value,
                    propOrientation, globalScratch)
                    : buildPropOrientation(x, y, z, props.refNode[prop], props.xNode[prop], props.yNode[prop],
                    props.baseRotationX[prop], props.baseRotationY[prop], props.baseRotationZ[prop],
                    props.rotationX[prop] * value, props.rotationY[prop] * value, props.rotationZ[prop] * value,
                    propOrientation);
            if (!props.originBound[prop]
                    || !buildReferenceBasis(x, y, z, props.refNode[prop], props.xNode[prop], props.yNode[prop], referenceBasis)
                    || !orientationReady) {
                // Degenerate authored reference frames are kept finite and invisible at the ref node.
                float cx = x[props.refNode[prop]], cy = y[props.refNode[prop]], cz = z[props.refNode[prop]];
            ...
            float refX = x[props.refNode[prop]], refY = y[props.refNode[prop]], refZ = z[props.refNode[prop]];

            float centerX = refX + bx0 * props.originLocalX[prop] + by0 * props.originLocalY[prop] + bz0 * props.originLocalZ[prop];
            float centerY = refY + bx1 * props.originLocalX[prop] + by1 * props.originLocalY[prop] + bz1 * props.originLocalZ[prop];
            float centerZ = refZ + bx2 * props.originLocalX[prop] + by2 * props.originLocalY[prop] + bz2 * props.originLocalZ[prop];
            ...
            float xScale = props.translationUseMeters[prop] ? 1.0f : distance(x, y, z, props.refNode[prop], props.xNode[prop]);
            float yScale = props.translationUseMeters[prop] ? 1.0f : distance(x, y, z, props.refNode[prop], props.yNode[prop]);
            float tx = props.translationX[prop] * value * xScale;
            float ty = props.translationY[prop] * value * yScale;
            float tz = props.translationZ[prop] * value;
    ...
    private static float functionValue(PropContainer props, SoftBodyVehicle vehicle, int prop, float steeringDegrees) {
        String name = props.function[prop];
        ...
        float transformed = (float) ((raw + props.offset[prop]) * props.multiplier[prop]);
        return Math.max(props.min[prop], Math.min(props.max[prop], transformed));
    }
